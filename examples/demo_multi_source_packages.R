...
cat("1. CRAN包安装演示\n")
cat("安装基础数据分析包...\n")
ensure_packages(c("dplyr", "ggplot2", "readr", "tidyr"))

# 2. Bioconductor包安装演示
cat("\n2. Bioconductor包安装演示\n")
cat("安装生物信息学包...\n")
# 注意：ensure_packages 会自动尝试从CRAN和Bioconductor安装包
ensure_packages(c("BiocManager", "Biobase"))
...
cat("尝试安装不存在的包...\n")
tryCatch({
  ensure_packages(c("nonexistent_package"), quiet = TRUE)
}, warning = function(w) {
  cat("捕获到警告:", w$message, "\n")
})
...
cat("\n9. 依赖处理演示\n")
cat("安装包及其依赖...\n")
ensure_packages(c("dplyr"), dependencies = TRUE)

cat("\n=== 多源包安装演示完成 ===\n")
cat("MSCRUtils 支持从以下来源安装包：\n")
cat("- CRAN: 使用 install.packages()\n")
cat("- Bioconductor: 自动安装BiocManager并使用BiocManager::install()\n")
cat("- 混合安装: 使用 install_from_multiple_sources() 一次性安装来自不同来源的包\n\n")

cat("主要功能特点：\n")
cat("✓ 自动检查包是否已安装\n")
cat("✓ 自动安装必要的工具（BiocManager）\n")
cat("✓ 完善的错误处理机制\n")
cat("✓ 支持静默模式\n")
cat("✓ 支持自定义仓库\n")
cat("✓ 支持依赖管理\n")

...
test_that("install_if_missing 参数验证测试", {
  # 测试缺失参数
  expect_error(install_if_missing(), "参数 'packages' 是必需的")
  
  # 测试NULL参数
  expect_error(install_if_missing(NULL), "参数 'packages' 是必需的")
  
  # 测试空向量
  expect_error(install_if_missing(character(0)), "参数 'packages' 必须是非空的字符向量")
  
  # 测试非字符向量
  expect_error(install_if_missing(1:5), "参数 'packages' 必须是非空的字符向量")
})
...
test_that("load_packages 参数验证测试", {
  # 测试缺失参数
  expect_error(load_packages(), "参数 'packages' 是必需的")
  
  # 测试NULL参数
  expect_error(load_packages(NULL), "参数 'packages' 是必需的")
  
  # 测试空向量
  expect_error(load_packages(character(0)), "参数 'packages' 必须是非空的字符向量")
  
  # 测试非字符向量
  expect_error(load_packages(1:5), "参数 'packages' 必须是非空的字符向量")
})
...
test_that("check_package_versions 参数验证测试", {
  # 测试缺失参数
  expect_error(check_package_versions(), "参数 'packages' 是必需的")
  
  # 测试NULL参数
  expect_error(check_package_versions(NULL), "参数 'packages' 是必需的")
  
  # 测试空向量
  expect_error(check_package_versions(character(0)), "参数 'packages' 必须是非空的字符向量")
  
  # 测试非字符向量
  expect_error(check_package_versions(1:5), "参数 'packages' 必须是非空的字符向量")
}) 